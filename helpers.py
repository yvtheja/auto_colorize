...
class Helpers:
    ...
    @staticmethod
    def toyuv(rgbimage): 
        with tf.name_scope('RGB_to_YUV') as scope:  
            filt = tf.constant([[[[0.299, -0.169, 0.499],[0.587, -0.331, -0.418],[0.114, 0.499, -0.0813]]]])
            bias = tf.constant([0., 0.5, 0.5])

            result = tf.nn.conv2d(rgbimage, filt, [1, 1, 1, 1], 'SAME')
    ...
    @staticmethod
    def torgb(yuvimage): 
        with tf.name_scope('YUV_TO_RGB'):

            yuvimage = tf.multiply(yuvimage, 255)
            filt = tf.constant([[[[1., 1., 1.],[0., -0.34413999, 1.77199996],[1.40199995, -0.71414, 0.]]]])
            bias = tf.constant([-179.45599365, 135.45983887, -226.81599426])

            result = tf.nn.conv2d(yuvimage, filt, [1, 1, 1, 1], 'SAME')
            result = tf.nn.bias_add(result, bias)

            result = tf.maximum(result, tf.zeros(result.get_shape(), dtype=tf.float32), name="pve_domain")
            result = tf.minimum(result, tf.multiply(tf.ones(result.get_shape(), dtype=tf.float32), 255), name="Normalizing")
            result = tf.div(result, 255)
